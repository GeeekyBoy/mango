...
The `cdn` option is used to define the CDN that you want to use to serve Mango runtime. The default value is `"self"`, which means that the runtime will be served from the same domain as your application. If you want to use a CDN, you can set the `cdn` option to `"jsdelivr"` or `"unpkg"`. If you set the `cdn` option to `"jsdelivr"`, the runtime will be served from `https://cdn.jsdelivr.net/npm/@mango-js/runtime@{version}/dist/mango.min.js`. If you set the `cdn` option to `"unpkg"`, the runtime will be served from `https://unpkg.com/@mango-js/runtime@{version}/dist/mango.min.js`. In both cases, `{version}` will be replaced with the version of the `@mango-js/scripts` package that is installed in your project.
