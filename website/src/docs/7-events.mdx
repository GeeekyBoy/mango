...
Mango supports all the events that are supported by the browser. You can attach event handlers to any element the smae way you would in other JSX-based frameworks. You can take advantage of the Mango-execlusive attributes accumulation feature to attach multiple event handlers to the same element.
...
- `onDestroy` - Triggered when the component is deattached from the DOM through Mango. Direct DOM manipulation will not trigger this event.
