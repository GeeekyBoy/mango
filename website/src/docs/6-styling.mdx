...
The simplest way to style your application is to use inline styles. Inline styles are styles that are applied directly to a component. You can define inline styles in 3 different ways. The first way is to use the `style` atribute and pass a string with CSS styles. This works just like the `style` attribute in HTML.
